...
def get_conversational_chain_offline(vector_store):
    sol_model = ChatOllama(model="Sol")  # Initialize the ChatOllama instance with the Sol model (Made for ARIA)
...
def get_conversational_chain_online(vector_store):
    llm = Together(model="mistralai/Mixtral-8x22B-Instruct-v0.1", temperature=0.7, max_tokens=1024)
    memory = ConversationBufferMemory(memory_key="chat_history", return_messages=True)
